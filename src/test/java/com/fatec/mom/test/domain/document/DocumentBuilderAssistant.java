...
public class DocumentBuilderAssistant {
    ...
    private static Set<Block> simpleBlocks() {
        ...
        var block1 = buildBlock(Block.builder().section("00").subSection("00").number(1).name("Capa1").code(10).order(1).status(BlockStatus.REVISED).traits(new HashSet<>()).build());
        block1.getTraits().add(simpleFiftyTrait());
        block1.getTraits().add(simpleFiftyFiveTrait());
        var block2 = buildBlock(Block.builder().section("01").subSection("00").number(2).name("Capa2").code(12).order(2).status(BlockStatus.REVISED).traits(new HashSet<>()).build());
        block2.getTraits().add(simpleFiftyTrait());
        block2.getTraits().add(simpleSixtyTrait());
        var block3 = buildBlock(Block.builder().section("02").subSection("00").number(3).name("Capa3").code(14).order(3).status(BlockStatus.REVISED).traits(new HashSet<>()).build());
        block3.getTraits().add(simpleFiftyTrait());
        block3.getTraits().add(simpleFiftyFiveTrait());
        block3.getTraits().add(simpleSixtyTrait());
        var block4 = buildBlock(Block.builder().section("03").subSection("00").number(4).name("Capa4").code(16).order(4).status(BlockStatus.REVISED).traits(new HashSet<>()).build());
        block4.getTraits().add(simpleFiftyTrait());
        block4.getTraits().add(simpleFiftyFiveTrait());
        block4.getTraits().add(simpleSixtyTrait());
    ...
    private static Block buildBlock(Block block) {
        block.setLinks(simpleLinks(block));
